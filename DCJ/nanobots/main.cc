...
void dispatch(LL l, LL r) {
    ...
    REP(i, M-1) {
        ...
        if (ll < 0) continue;
        if (total < N) {
            //printf("In dispatch(%lld,%lld): received %lld %d, dispatching(%lld, %lld)\n", l, r, ll, cnt, ll, sent[i]);
            dispatch(ll, sent[i]);
        }
    }
}
