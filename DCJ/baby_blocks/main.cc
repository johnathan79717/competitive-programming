...
int main() {
    int M = NumberOfNodes() - 1, id = MyNodeId();
    int N = GetNumberOfBlocks();
    if (id < M) {
        LL sum = 0;
        LL lb = N * id / M, ub = N * (id + 1) / M - 1;
        for (LL i = lb; i <= ub; i++) {
            sum += GetBlockWeight(i);
        }
        putLL(M, sum);
        send(M);
        receive(M);
        int i = getInt(M), j = getInt(M);
        LL x = i * N / M, y = (j+1) * N / M - 1;
        LL prefix = getLL(M) + GetBlockWeight(x);
        LL suffix = getLL(M) + GetBlockWeight(y);
        int ans = 0;
        while (x < N * (i+1) / M && y >= N * j / M && x < y) {
            if (prefix < suffix) {
                prefix += GetBlockWeight(++x);
            } else if (prefix > suffix) {
                suffix += GetBlockWeight(--y);
            } else {
                ans++;
                prefix += GetBlockWeight(++x);
                suffix += GetBlockWeight(--y);
        ...
        int i = 0, j = M-1;
        LL prefix = sums.front(), suffix = sums.back();
        while (i <= j) {
            if (prefix < suffix) {
                putInt(i, i);
                putInt(i, j);
                putLL(i, prefix - sums[i]);
                putLL(i, suffix - sums[j]);
                send(i);
                prefix += sums[++i];
            } else {
                putInt(j, i);
                putInt(j, j);
                putLL(j, prefix - sums[i]);
                putLL(j, suffix - sums[j]);
                send(j);
                suffix += sums[--j];
            }
        }
