...
LL lower_bound(function<LL(LL)> get, LL L, LL x) {
  LL lb = -1, ub = L;
  while (ub - lb > 1) {
    LL mid = (ub - lb) / 2 + lb;
    if (get(mid) >= x) {
...
pair<LL, LL> binary_search(LL x) {
  LL lb = 0, ub = 5000000002ll;
  LL i, j;
  while (ub - lb > 1) {
    LL m = (lb + ub) / 2;
