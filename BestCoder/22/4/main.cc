...
bool compare(Query const &p1, Query const &p2) {
    return p1.R < p2.R;
}
...
int main() {
    ...
    while(T--) {
        ...
        REP(i, M) {
            ...
            query[i].i = i;
            group[(query[i].L - 1) / 200].PB(query[i]);
        }
        REP(i, 150) {
            sort(ALL(group[i]), compare);
            int l = 200*i + 1, r = l;
            int total = 0;
            MS0(each);
            Z ans = 1;
            for (vector<Query>::iterator it = group[i].begin(); it != group[i].end(); it++) {
                while (r <= it->R) {
                    ans *= (++total);
                    ans /= (++each[a[r++]]);
                }
