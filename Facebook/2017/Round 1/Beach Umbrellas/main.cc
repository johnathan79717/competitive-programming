...
Z cnt[4005], cnt1[4005];

Z H(int a, int b) {
  if (a < 0) return 0;
  return Z::choose1(a + b-1, b-1);
}
...
int main() {
  ...
  while(T--) {
    ...
    int i = max(sum - M + 1, 1);
    Z h1 = Z::choose1(M-1+i-sum+N-1, N-1);
    Z h = Z::choose1(M-1+i-sum+N-2, N-2);
    while (i < 4005) {
      ans += h1 * cnt1[i] * 2 * Z::fact(N-1);
      ans += h * cnt[i] * 2 * Z::fact(N-2);
      h1 = h1 * (M+i-sum+N-1) / (M+i-sum);
      h = h * (M+i-sum+N-2) / (M+i-sum);
      i++;
    }
    PZ(ans);
  }
