...
int main() 
{
    ...
    V(Query) query(Q);
    REP(i, Q) {
        RI(query[i].l, query[i].r);
        query[i].i = i;
    }

    const int B = 4500;
    sort(ALL(query), [](const Query& p1, const Query& p2) {
        int b1 = p1.l / B, b2 = p2.l / B;
        if (b1 != b2) return b1 < b2;
        else if (b1 % 2) return p1.r > p2.r;
        else return p1.r < p2.r;
    });
    
    int l = 1, r = 1;
    Z prod = 1;
    auto add = [&A,&prod](int i) {
        prod *= A[i];
        int a = A[i];
        while (a > 1) {
            int p = prime[a];
