...
Z solve(int a, int b) {
    debug(a, b);
    if (a > 1) {
        auto it = portal.lower_bound(a);
        if (it->F != a) {
            int x = it == portal.begin() ? 1 : (--it)->F;
            mem[a*MOD+b] = solve(x, b) - a + x;
            debug(a, b, mem[a*MOD+b]);
            return mem[a*MOD+b];
        }
    }
    auto j = mem.find(a * MOD + b);
    if (j != mem.end()) {
        debug("mem");
        debug(a, b, j->S);
        return j->S;
    }
    auto it = portal.lower_bound(b);
    if (it == portal.begin() || (--it)->F <= a) {
        mem[a * MOD + b] = b - a;
        debug(a, b, mem[a*MOD+b]);
        return mem[a*MOD+b];
    }
    int y = it->F, x = it->S.F, s = it->S.S;
    if (a == 0 && !s) {
        mem[a * MOD + b] = solve(0, y) + b - y;
        debug(a, b, mem[a*MOD+b]);
        return mem[a * MOD + b];
    }
    mem[a * MOD + b] = solve(a, y) + solve(x, y) + b - y;
    debug(a, b, mem[a*MOD+b]);
    return mem[a * MOD + b];
}
