...
double solve(int r, int c) {
  ...
  if(r < n && c < n)
    return dp[r][c] = (n*n + solve(r+1, c) * (n-r) * c + solve(r, c+1) * r * (n-c) + solve(r+1, c+1) * (n-r) * (n-c)) / (n * n - r * c);
  if(c < n)
    return dp[n][c] = solve(n, c+1) + double(n) / (n-c);
  else
    return dp[r][n] = solve(r+1, n) + double(n) / (n-r);
}
