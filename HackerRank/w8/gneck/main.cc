...
struct Matrix {
  ...
  Matrix &operator*= (const Matrix &other) {
    ...
    REP(i, 3) REP(j, 3) a[i][j] = temp.a[i][j];
    return *this;
  }

  Matrix pow(LL b) {
    Matrix x,y=*this; // ll is taken to avoid overflow of intermediate results
    x.a[0][0] = x.a[1][1] = x.a[2][2] = 1;
    while(b > 0){
      if(b%2 == 1)
        x *= y;
      y *= y; // squaring the base
...
int main() {
  ...
  while(T--) {
    DRL(N);
    Matrix m;
    m.a[0][0] = m.a[0][1] = m.a[1][2] = m.a[2][0] = 1;
    PI(m.pow(N+2).a[0][0].i);
  }
  return 0;
}