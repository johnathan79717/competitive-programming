...
int main() {
    ...
    REP1(i, 0, x) {
        REP1(j, 0, y) {
            REP1(k, 0, z) {
                if (i + j + k) {
                    Z q = (Z(10).pow(i+j+k) - 1) / 9;
                    if (i)
                        ans += q * Z::fact(i+j+k-1) * (Z::inv_fact(i-1) * Z::inv_fact(j) * Z::inv_fact(k)) * 4;
                    if (j)
                        ans += q * Z::fact(i+j+k-1) * (Z::inv_fact(i) * Z::inv_fact(j-1) * Z::inv_fact(k)) * 5;
                    if (k)
                        ans += q * Z::fact(i+j+k-1) * (Z::inv_fact(i) * Z::inv_fact(j) * Z::inv_fact(k-1)) * 6;
                }
            }
        }
    }
