    ...
    if (u != p) {
      if (p == -1) {
        if (adj[v].size() == 1)
          ...
          up[u] = ((down[v] - T[v]) * adj[v].size() - down[u]) / (adj[v].size() - 1) + T[v];
        }
      } else {
        up[u] = down[v] + (up[v] - down[u]) / (adj[v].size() - 1);
      }
      dfs2(u, v);
    }
  }
...
int main() {
  ...
  double t = down[1];
  REP1(i, 2, N) {
    double t2 = T[i] + ((down[i] - T[i]) * (adj[i].size() - 1) + up[i]) / adj[i].size();
    if (t2 < t) {
      t = t2;
      ans = i;
    }
