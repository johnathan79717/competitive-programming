        ...
        REP(j, M) {
            ...
            for(int k: stateNums) {
                ...
                auto block = [&]() {
                    ...
                    if (cnt == 1) { // j's set has only j, so a tree is isolated
                        bool fail = false;
                        REP(i, M) {
                            if (a[i] > 1) { // there are other trees growing, won't be connected
                                fail = true;
                                break;
                            }
                        }
                        ...
                        if (!h && !fail) { // there can't be roads left in other cells
                            // fill all the left cells with trees
                            ans += dp[n][k];
                        }
                    } else if (first == j) { // j is the first of its group
                        int l = a[j], m = a[j];
                        int i;
                        for(i = j+1; i < M; i++) {
                            if (a[i] == l) {
                        ...
                        // m is the largest number between j and the next of j's group
                        // the number between a[j] and m will decrease,
                        REP(i, M) {
                            if (a[i] == l) {
                                a[i] = m;
                            } else if (a[i] > l && a[i] <= m) {
                ...
                if (state[j]) { // up cell is road
                    if (s[i][j] != '.') { // don't build road
                        // remove j from j's set
                        block();
                    }
                    if (s[i][j] != '#') { // build road
                        if (j == 0 || state[j-1] == 0) { // left is not road
                            dp[n^1][k] += dp[n][k];
                        } else { // left is road
                            // connect j-1 and j
