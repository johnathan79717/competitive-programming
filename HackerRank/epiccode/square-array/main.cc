...
struct Lazy {
    Z i0, i1, i2;
    Lazy() : i0(0), i1(0), i2(0) {}
    Lazy(Z i2, Z i1, Z i0): i0(i0), i1(i1), i2(i2) {}

    void clear() {
        i0 = i1 = i2 = 0;
    }

    Lazy& operator+=(const Lazy& d) {
        i0 += d.i0;
        i1 += d.i1;
        i2 += d.i2;
        return *this;
...
Z inv6 = Z(6).inverse(), inv2 = Z(2).inverse();
...
struct SegmentTree {
  ...
  void propagate(int k, Lazy &d, int l, int r) {
    // dat[k] += d;
    dat[k] += (d.i0 + d.i1 * (l + r - 1) * inv2) * (r - l) + d.i2 * inv6 * (Z(r-1) * r * (2 * r - 1) - Z(l-1) * l * (2 * l - 1));
    if (chr(k) < lazy.size()) {
        lazy[chl(k)] += d;
        lazy[chr(k)] += d;
    }
  ...
  void update(int a, int b, int k, int l, int r, Lazy& d) {
    if (lazy[k].i0.i || lazy[k].i1.i || lazy[k].i2.i) {
