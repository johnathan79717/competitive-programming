...
struct DisjointSet {
    ...
    void unite(int x, int y) {
        ...
        if(_size[x] < _size[y]) {
            par[x] = y;
            mini(left[y], left[x]);
            maxi(right[y], right[x]);
            _size[y] += _size[x];
        } else {
            mini(left[x], left[y]);
            maxi(right[x], right[y]);
            par[y] = x;
            _size[x] += _size[y];
        }
    }
...
int main() {
    ...
    DisjointSet set(N);
    auto segments = [](int l) {
        return Z::choose0(l+1, 2);
    };
    Z total = 0, ans = 0;
    for (auto &pr: A) {
        int i = pr.Y, a = pr.X;
        if (i > 0 && used[i-1]) {
            total -= segments(set.size(i-1));
            set.unite(i, i-1);
        }
        if (i+1<N && used[i+1]) {
            total -= segments(set.size(i+1));
        ...
        int l = i - set.getLeft(i) + 1, r = set.getRight(i) - i + 1;
        ans += total * l * r * a;
        ans += (Z::choose0(l+1, 3) * r + Z::choose0(r+1, 3) * l) * a;
        used[i] = true;
        total += segments(set.size(i));
    }
    PZ(ans);
    return 0;
}
