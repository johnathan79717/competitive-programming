...
int main() {
  ...
  while(T--) {
    ...
    REP(i, N) RI(A[i]);
    sort(ALL(A));
    V(V(Z)) e(N+1, V(Z)(N+1)), o = e, c = o;
    REP1(i, 0, N) c[i][i] = 1;
    REP1(l, 1, N) {
      REP1(i, 0, N-l) {
        REP1(j, i, i+l-1) {
          c[i][i+l] += c[i][j] * c[j+1][i+l];
          e[i][i+l] += o[i][j] * c[j+1][i+l] + o[j+1][i+l] * c[i][j] + c[i][j] * c[j+1][i+l] * A[j];
          o[i][i+l] += e[i][j] * c[j+1][i+l] + e[j+1][i+l] * c[i][j];
        }
      }
    }
    PI((e[0][N] * a - o[0][N] * b).i);
  }
  return 0;
}