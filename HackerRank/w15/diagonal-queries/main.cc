...
struct SegmentTree {
  ...
  void update(int a, int b, AP ap) {
    update(a, b+1, 0, 0, n, ap);
  }

  void propagate(int k, AP &ap, int l, int r) {
    dat[k] += (ap.a + ap.d * l + ap.a + ap.d * (r-1)) * (r - l) / 2;
    if (chr(k) < lazy.size()) {
        lazy[chl(k)] += ap;
        lazy[chr(k)] += ap;
    }
  ...
  void update(int a, int b, int k, int l, int r, AP ap) {
    if (!lazy[k].empty()) {
    ...
    if (a <= l && r <= b) {
        ...
        update(a, b, chl(k), l, (l+r)/2, ap);
        update(a, b, chr(k), (l+r)/2, r, ap);
    ...
    while (Q--) {
        ...
        switch (type[1]) {
            ...
            case 'r': {
                ...
                ap.d = d;
                ap.a = ap.d * (1 - r);
                st.update(r, r+M-1, ap);
                break;
            }
            case 's': {
                DRI(x1, y1, x2, y2, d);
                int dx = x2 - x1 + 1, dy = y2 - y1 + 1;
                y1--; y2--; x1 = N - x1; x2 = N - x2; swap(x1, x2);
                if (dx < dy) {
                    AP ap;
                    ap.d = d;
                    int D = x1 + y1;
                    ap.a = ap.d * (1 - D);
                    st.update(D, D + dx - 2, ap);

                    ap.d = -d;
                    D = x2 + y2;
                    ap.a = ap.d * (-1 - D);
                    st.update(D - dx + 2, D, ap);

                    D = x2 + y1;
                    ap.d = 0;
                    ap.a = Z(d) * dx;
                    st.update(D, D + dy - dx + 1 - 1, ap);
                } else {
                    AP ap;
                    ap.d = d;
                    int D = x1 + y1;
                    ap.a = ap.d * (1 - D);
                    st.update(D, D + dy - 2, ap);

                    ap.d = -d;
                    D = x2 + y2;
                    ap.a = ap.d * (-1 - D);
                    st.update(D - dy + 2, D, ap);

                    D = x1 + y2;
                    ap.d = 0;
                    ap.a = Z(d) * dy;
                    st.update(D, D + dx - dy, ap);
                }
            }
            default:
                break;
