...
unordered_set<int> out[1000001];
unordered_set<int> in[1000001];
bool exists[1000001];
...
int main() {
  ...
  REP(i, N) {
    ...
    REP(j, K-1) {
      out[A[i][j]].insert(A[i][j+1]);
      in[A[i][j+1]].insert(A[i][j]);
    }
  }
  PQ<int, vector<int>, greater<int>> candidates;
  REP1(i, 1, 1000000) {
    if (exists[i] && in[i].empty()) {
      candidates.push(i);
    }
  }
