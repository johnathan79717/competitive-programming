...
struct Z {
  ...
  Z():i(0) {}
  Z(int i): i(i >= 0 ? i : i + M) {}
  void operator +=(const Z& z) { i += z.i; if(i >= M) i -= M; }
  void operator -=(const Z& z) { i -= z.i; if(i < 0) i += M; }
  void operator *=(const Z& z) { i = (LL) i * z.i % M; }
...
LL f(LL n) {
  int M30 = M * 30;
  n %= M30;
  return n * (n+1) % M30 * (n * 2 + 1) % M30 *
    (n * n * 3 % M30 + n * 3 - 1) % M30 / 30;
}
