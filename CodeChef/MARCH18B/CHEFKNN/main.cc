...
struct ZZ {
    ...
    void operator -=(const ZZ& z) { i >= z.i ? i -= z.i : i = i + MOD - z.i; }
...
typedef ZZ<unsigned, unsigned long long> Z;
template<> unsigned Z::MOD = 1000000007;
...
int main() {
    for (int n = 0; n < 2505; n++) {
        for (int k = 0; k < 2505; k++) {
            if (n == 0 || k == 0) {
                dp[n][k] = 1u;
            } else {
                dp[n][k] = dp[n-1][k-1] * unsigned(n-1) + dp[n-1][k] + dp[n][k-1];
            }
        }
    }
