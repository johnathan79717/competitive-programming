...
int main() {
  ...
  while(T--) {
    ...
    V(V(VI)) dp(N+1, V(VI)(K+1, VI(max_p+1)));
    dp[0][1][P[0]] = 1;
    REP1(i, 1, N-1) {
      REP1(k, 0, K) {
        addBy(dp[i][k][P[i]], mul(i+1, dp[i-1][k][P[i]]));
        REP1(m, P[i]+1, max_p) {
          if(k < K)
            addBy(dp[i][k+1][P[i]], dp[i-1][k][m]);
          addBy(dp[i][k][m], mul(i, dp[i-1][k][m]));
        }
      }
    }
    int ans = 0;
    REP1(k, 0, K) REP1(m, 0, max_p)
      addBy(ans, dp[N-1][k][m]);
    PI(ans);
  }
  return 0;
}