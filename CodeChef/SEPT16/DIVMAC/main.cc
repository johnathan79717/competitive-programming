...
struct SegmentTree {
  ...
  void init(int k, int l, int r) {
    if(r - l == 1) {
      ...
      init(chl(k), l, m);
      init(chr(k), m, r);
      combine(dat[k], dat[chl(k)], dat[chr(k)]);
    }
  }
  ...
  int query(int a, int b, int k, int l, int r) {
    ...
    if(a <= l && r <= b) {
      ...
      int vl = query(a, b, chl(k), l, (l + r) / 2);
      int vr = query(a, b, chr(k), (l + r) / 2, r);
      return max(vl, vr);
    }
  }
};
