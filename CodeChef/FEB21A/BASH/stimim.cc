...
void solve() {
    int N, Pu, Pl, Pd, Pr;
    R(N, Pu, Pl, Pd, Pr);
    atcoder::mcf_graph<int, int> graph(N * N + 2), g01(N*N+2);
        ...
        FOR(j, 1, N) {
            ...
            if (r[i][j] == i && c[i][j] == j) {
                ...
                if ((i+j) % 2) {
                    ...
                    g01.add_edge(node, t, 1, 0);
                } else {
                    graph.add_edge(node, t, 1, 0);
                    g01.add_edge(s, node, 1, 0);
                }

                if (i > 1 && r[i-1][j] == i-1 && c[i-1][j] == j) {
                    if ((i+j) % 2) {
                        graph.add_edge(node, node - N, 1, Pu);
                        g01.add_edge(node - N, node, 1, Pd);
                    } else {
                        graph.add_edge(node - N, node, 1, Pd);
                        g01.add_edge(node, node - N, 1, Pu);
                    }
                }
                if (j > 1 && r[i][j-1] == i && c[i][j-1] == j-1) {
                    if ((i+j) % 2) {
                        graph.add_edge(node, node-1, 1, Pl);
                        g01.add_edge(node-1, node, 1, Pr);
                    } else {
                        graph.add_edge(node-1, node, 1, Pr);
                        g01.add_edge(node, node-1, 1, Pl);
                    }
                }
            }
            node++;
    ...
    auto pr = graph.flow(s, t), pr01 = g01.flow(s, t);
    if (pr.F * 2 != cycleNodes || pr01.F * 2 != cycleNodes) {
        W(-1);
        return;
    }
    vector<string> ans(N+1, string(N+1, '.'));
    auto processEdges = [&](auto const &edges) {
        for (auto &e : edges) {
    ...
    processEdges(graph.edges());
    processEdges(g01.edges());
