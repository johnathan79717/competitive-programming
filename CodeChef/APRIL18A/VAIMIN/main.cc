...
Z f(const PII &a, const PII &b) {
    int cx = a.Y + c - 1, cy = a.X - c + 1;
    return Z::choose0(b.Y - a.Y + b.X - a.X, b.X - a.X) - Z::choose0(b.Y - cy + b.X - cx, b.X - cx);
}
...
int main() {
    ...
    V(Z) dp(SZ(gb)+1);
    REP(i, SZ(gb)) {
        dp[i+1] = f(MP(c+1, 0), gb[i]);
        REP(j, i) {
            dp[i+1] -= dp[j+1] * f(gb[j], gb[i]);
        }
    }
    PZ(dp.back());
    return 0;
