...
struct ZZ {
    ...
    ZZ inverse() const {
        if (i < SZ(inv_factorial)) {
            return inv_factorial[i] * factorial[i-1];
        }
        return pow(MOD - 2);
    }
    ...
    static vector<ZZ> factorial, inv_factorial, inv, df, inv_df;
    ...
    static ZZ double_fact(int n) {
        if (n < 0) return 1;
        while (df.size() <= n) {
            df.push_back(df[SZ(df)-2] * SZ(df));
        }
        return df.at(n);
    }

    static ZZ inv_double_fact(int n) {
        if (n < 0) return 1;
        if (n < inv_df.size())
            return inv_df.at(n);
        int old_size = inv_df.size();
        inv_df.resize(n+1);
        inv_df.at(n) = double_fact(n).inverse();
        inv_df.at(n-1) = double_fact(n-1).inverse();
        for (int i = n-2; i >= old_size; i--) {
            inv_df[i] = inv_df.at(i+2) * (i+2);
        }
        return inv_df.at(n);
    }
...
template<> vector<Z> Z::df(2, 1);
template<> vector<Z> Z::inv_df(2, 1);
...
int main() {
    DRI(N, K, A, B);
    Z a = A % Z::MOD, b = B % Z::MOD, k = K % Z::MOD;
    V(Z) dp(N+1);
    Z::inv_fact(N);
    dp[0] = b.inverse() / (-2);
    dp[1] = k + a / b / 2;
    REP1(n, 0, N-2) {
        dp[n+2] = ((a+b*k*2)*(1+2*n)*dp[n+1] - a*a*(n-1)*dp[n]) / (n+2);
    }
    dp[0] = 0;
    dp[1] = k * k;
    REP1(n, 2, N) {
        dp[n] = dp[n] * dp[n] + dp[n-1];
    }
    DRI(Q);
    while (Q--) {
        DRI(L, R);
