...
void heavylight_BFS(int N, int root = 0)
{
    ...
    *qb++ = root;
    largest[root] = A[root];
    while (qf < qb)
        for (int i=*qf++, k=0; k<SZ(adj[i]); ++k)
        {
            int j = adj[i][k];
            if (j == parent[i]) continue;
            parent[j] = i;
            largest[j] = max(largest[i], A[k]);
            depth[j] = depth[i] + 1;
            *qb++ = j;
        }
    ...
    for (int i=0; i<N; ++i)
        if (parent[i] == -1 || heavy[parent[i]] != i)
        {
            ...
            for (int k = i; k != -1; k = heavy[k]) {
                chain[k] = c, head[k] = i;
                if (parent[k] == -1 || A[k] > largest[parent[k]]) {
                    seq.PB(A[k]);
                }
                pos[k] = seq.size();
            }
...
int main() {
    CASET {
        ...
        while (Q--) {
            ...
            while (v >= 0) {
                int c = chain[v];
                int k = upper_bound(seqs[c].begin(), seqs[c].begin() + pos[v], W)
                        - seqs[c].begin();
                ans += pos[v] - k;
                if (k > 0) {
                    break;
