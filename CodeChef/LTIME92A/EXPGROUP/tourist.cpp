...
int main() {
  ...
  Mint ans = 0;
  vector<vector<Mint>> c(n + 1);
  vector<vector<Mint>> p(n + 1);
  for (int i = 1; i <= n; i++) {
    c[i] = vector<Mint>{0, 1 / Mint(i)};
    p[i] = vector<Mint>{1, -Mint(i - 1) / i};
  }
  c[0] = p[0] = vector<Mint>{1};
  vector<Mint> den = multiply(p);
  function<vector<Mint>(int, int)> Solve = [&](int from, int to) {
    if (from == to) {
      return c[from] + p[from];
    }
    int mid = (from + to) >> 1;
    auto R = multiply(vector<vector<Mint>>(p.begin() + mid + 1, p.begin() + to + 1));
    auto L = multiply(vector<vector<Mint>>(c.begin() + from, c.begin() + mid + 1));
    auto res = Solve(from, mid) * R;
    res += Solve(mid + 1, to) * L;
    res -= L * R;
    return res;
  };
  vector<Mint> num = Solve(2, n) * c[1];
  num = num * inverse(den);
  for (int i = 2; i <= n; i++) {
    ans += num[i] * (i - 1);
  }
