...
void solve() {
    ...
    function<pair<vector<Mint>, vector<Mint>>(int, int)> Solve = [&](int l, int r) {
        if (l == r) {
            return MP(
                    vector<Mint>{0, Mint(1) / (l+1)},
                    vector<Mint>{l, -Mint(l) * Mint(l) / Mint(l + 1)}
                    );
        }
        int m = (l + r) >> 1;
        auto pr1 = Solve(l, m), pr2 = Solve(m+1, r);
        pr2.F.insert(pr2.F.begin(), m-l+1, 0);
        return MP(pr1.F * pr2.S + pr2.F, pr1.S * pr2.S);
    };
    auto s = Solve(1, N-1);
    auto G = s.F * inverse(s.S);
    debug(G);
    Mint ans = 0;
    REP(i, N) {
        ans += G[i] * i;
