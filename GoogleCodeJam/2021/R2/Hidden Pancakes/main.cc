...
Z solve(int l, int r, int m) {
    if (l == r) {
        return A[l] == m ? 1 : 0;
    }
    auto it = upper_bound(ALL(pos[m]), r);
    if (it == pos[m].begin()) {
        return 0;
    }
    int i = *--it;
    ...
    if (i == l) {
        return solve(l+1, r, m+1);
    }
    if (i == r) {
        return solve(l, r-1, m);
    }
    return Z::choose0(r - l, i - l) * solve(l, i-1, m) * solve(i+1, r, m+1);
}
