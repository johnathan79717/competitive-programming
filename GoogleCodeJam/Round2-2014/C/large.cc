...
int main() {
  ...
  REP1(t, 1, T) {
    ...
    while(i < e.size()) {
      int y = e[i].F;
      if(prev > 0 && cnt == 0 && y - prev - 1 > 0) {
        gap[c++] = y - prev - 1;
      }
      while(i+1 < e.size() && e[i+1].F == y && e[i+1].S) {
        cnt++;
        i++;
      }
      while(i+1 < e.size() && e[i+1].F == y) {
        cnt--;
        i++;
      }
      mp[y] = c++;
    ...
    REP(i, B) {
      // b[i].Y0 = lower_bound(ALL(ys), b[i].Y0) - ys.begin();
      // b[i].Y1 = lower_bound(ALL(ys), b[i].Y1) - ys.begin();
      b[i].Y0 = mp[b[i].Y0];
      b[i].Y1 = mp[b[i].Y1];
      REP1(x, b[i].X0, b[i].X1) REP1(y, b[i].Y0, b[i].Y1) {
        building[x][y] = true;
      }
    }
    Graph g;
    ...
    REP(x, W) REP(y, H) {
      ...
      out[x][y] = g.addVertex();
      int cap = 1;
      if(gap.count(y)) cap = gap[y];
      if(y == 0)
        g.addEdge(g.source, in[x][y], 1);
      if(y == H-1)
        g.addEdge(out[x][y], g.sink, 1);
      g.addEdge(in[x][y], out[x][y], cap);
      if(x > 0 && !building[x-1][y]) {
        g.addEdge(out[x-1][y], in[x][y], cap);
        g.addEdge(out[x][y], in[x-1][y], cap);
      }
